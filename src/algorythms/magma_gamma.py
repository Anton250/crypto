...
class MagmaGamma:
    ...
    def _encrypt_function(self, part:int, key:int):
        ...
        Функция шифрования
        '''
        temp_val = bin((part + key) % int(pow(2, 32)))[2:].zfill(32) # складываем по модулю 2^32
        result = ''
        for i in range(8):
            # производим простую замену по таблице
            result += bin(SUBSITUTION_BLOCK[i + 1][SUBSITUTION_BLOCK[0].index(int(temp_val[i * 4:i * 4 + 4],2) % 16)])[2:].zfill(4)
        result = [i for i in result]
        result[:32 - 11], result[32 - 11:] = result[11:], result[:11] # циклический сдвиг на 11 бит
    ...
    def encrypt(self, mes):
        try:
            mes = bin(int(mes, 16))[2:]
        except:
            mes = bin(int(bytes(mes, 'utf-8').hex(), 16))[2:]
        ...
        blocks = [
            mes[i * 64:i * 64 + 64]
            for i in range(ceil(len(mes)/64))
        ]
        ...
        for block in blocks:
            ...
            N4 = bin((int(N4,2) + C1) % int((pow(2, 32) - 1)))[2:].zfill(32)
            # складываем значение из N3 с константой C2 по модулю 2^32
            N3 = bin((int(N3,2) + C2) % int(pow(2, 32)))[2:].zfill(32)
            N1, N2 = N3, N4
            ...
            # XOR гаммы с блоком
            encrypted += bin(int(gamma, 2) ^ int(block, 2))[2:].zfill(len(block))
    ...
    def decrypt(self, mes):
        ...
        N1, N2 = self.synchro[:32], self.synchro[32:]
        N3, N4 = self._encrypt_subsitution(N1, N2)
        ...
        for block in blocks:
            ...
            if len(gamma) > len(block):
                gamma = gamma[:len(block)]
            decrypted += bin(int(gamma, 2) ^ int(block, 2))[2:].zfill(len(block))
